...
function TestGenID()
{
    global $db;
    $PrevDebug = $db->debug;
    $db->debug = false; // Hide debug if present as Drop can cause errors
...
");
    ...
    if (intval($ID1a) + 1 !== intval($ID1b)) {
        DieTrace(sprintf("ERROR : Second value obtains by MySequence1 should be %d but is %d", $ID1a + 1, $ID1b));
    }

    $db->CreateSequence("MySequence3", 100);
    $ID2b = $db->GenID("MySequence3");
    if (intval($ID2b) !== 100) {
        DieTrace(sprintf("ERROR : Value from MySequence3 should be 100 but is %d", $ID2b));
    }
    ...
    if (intval($ID1a + 1) !== intval($ID1b)) {
        DieTrace(sprintf("ERROR : Second value obtains by MySequence1Emul should be %d but is %d", $ID1a + 1, $ID1b));
    }

    $db->CreateSequence("MySequence3Emul", 100);
    $ID2b = $db->GenID("MySequence3Emul");
    if (intval($ID2b) !== 100) {
        DieTrace(sprintf("ERROR : Value from MySequence3Emul should be 100 but is %d", $ID2b));
    }
} //TestGenID()
