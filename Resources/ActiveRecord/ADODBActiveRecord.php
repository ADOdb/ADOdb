...
class ADODBActiveRecord
{
	...
	var $_tableat; // associative index pointing to ADODBActiveTable, eg $ADODB_Active_DBS[_dbat]->tables[$this->_tableat]
	...
	function __construct($table = false, $pkeyarr=false, $db=false)
	{
		...
		if ($db) {
			$this->_dbat = ADODBActiveRecord::SetDatabaseAdapter($db);
			...
			if (sizeof($_ADODB_ACTIVE_DBS) == 0) {
				$this->Error(
					...
					'ADODBActiveRecord::__constructor'
				);
			}
	...
	{
		if (!ADODBActiveRecord::$_changeNames) {
			return $table;
		}
	...
	function hasMany(
		$foreignRef, 
		$foreignKey = false, 
		$foreignClass = '\ADOdb\Resources\ActiveRecord\ADODBActiveRecord'
		) {
		$ar = new $foreignClass($foreignRef);
		$ar->foreignName = $foreignRef;
		$ar->UpdateActiveTable();
		$ar->foreignKey = $foreignKey ?: $foreignRef.ADODBActiveRecord::$_foreignSuffix;
	...
	static function TableHasMany($table, $foreignRef, $foreignKey = false, 
	$foreignClass = '\ADOdb\Resources\ActiveRecord\ADODBActiveRecord'
	)
	{
		$ar = new ADODBActiveRecord($table);
		$ar->hasMany($foreignRef, $foreignKey, $foreignClass);
	}

	// use when you don't want ADOdb to auto-pluralize tablename
	static function TableKeyHasMany($table, $tablePKey, $foreignRef, $foreignKey = false, 
	$foreignClass = '\ADOdb\Resources\ActiveRecord\ADODBActiveRecord'
	...
	)
	{
		...
		$ar = new ADODBActiveRecord($table, $tablePKey);
		$ar->hasMany($foreignRef, $foreignKey, $foreignClass);
	}
	...
	static function ClassHasMany($parentclass, $foreignRef, $foreignKey = false, 
	$foreignClass = '\ADOdb\Resources\ActiveRecord\ADODBActiveRecord'
	)
	{
		$ar = new $parentclass();
	...
	$parentClass = '\ADOdb\Resources\ActiveRecord\ADODBActiveRecord'
	)
	{
	...
	static function ClassBelongsTo($class, $foreignRef, $foreignKey = false, $parentKey = '', $parentClass =  '\ADOdb\Resources\ActiveRecord\ADODBActiveRecord')
	{
		$ar = new $class();
		$ar->belongsTo($foreignRef, $foreignKey, $parentKey, $parentClass);
	}

	static function TableBelongsTo($table, $foreignRef, $foreignKey = false, $parentKey = '', $parentClass =  '\ADOdb\Resources\ActiveRecord\ADODBActiveRecord')
	{
		$ar = new ADODBActiveRecord($table);
		$ar->belongsTo($foreignRef, $foreignKey, $parentKey, $parentClass);
	}

	static function TableKeyBelongsTo($table, $tablePKey, $foreignRef, $foreignKey = false, $parentKey = '', $parentClass =  '\ADOdb\Resources\ActiveRecord\ADODBActiveRecord')
	{
		if (!is_array($tablePKey)) {
			$tablePKey = array($tablePKey);
		}
	...
	/**
	 * @param string $name
	 * @param string $whereOrderBy : e.g. ' AND field1 = value ORDER BY field2'
	 * @param int $offset
	 * @param int $limit
	 *
	 * @return mixed
	...
	function UpdateActiveTable($pkeys=false, $forceUpdate=false)
	{
		...
		$activetab = new ADODBActiveTable();
	...
	/**
	 * Retrieve ADODBActiveTable.
	 *
	 * @return ADODBActiveTable
	 */
	function &TableInfo()
	{
		global $_ADODB_ACTIVE_DBS;
		$activedb = $_ADODB_ACTIVE_DBS[$this->_dbat];
		return $activedb->tables[$this->_tableat];
	}
	...
	function LastInsertID($db, $fieldname)
	{
		...
		{
			$SQL = sprintf(/** @lang text */ "SELECT MAX(%s) FROM %s",
	...
	function Load($where = null, $bindarr = false, $lock = false)
	{
		...
		$qry = sprintf(/** @lang text */ "SELECT * FROM %s",
					   $this->nameQuoter($db,$this->_table)
					   );
	...
	function Insert()
	{
		...
		$sql = sprintf(/** @lang text */ 'INSERT INTO %s (%s) VALUES (%s)',
					   $tableName,
					   implode(',',$names),
					   implode(',',$valstr)
					   );
	...
	function Delete()
	{
		...
		$sql = sprintf(/** @lang text */ 'DELETE FROM %s WHERE %s',
					   $tableName,
					   $where
					   );
	...
	function Find($whereOrderBy, $bindarr = false, $pkeysArr = false, $extra = array())
	{
		...
		return $db->GetActiveRecordsClass(get_class($this),$this->_table, $whereOrderBy,$bindarr,$pkeysArr,$extra);
	}

	/**
	 * @return false|int 0 on error, 1 on update, 2 on insert
	 */
	function Replace()
	{
		$db = $this->DB();
	...
	/**
	 * @return false|int 0 on error, 1 on update, -1 if no change in data (no update)
	 */
	function Update()
	{
		$db = $this->DB();
		...
		if (!$where) {
			$this->error("Where missing for table $table->name", "Update");
			return false;
		}
		$valarr = array();
		$neworig = array();
	...
	private function nameQuoter($db, $name)
	{
		...
		$helper = new \ADOdb\Resources\ADOHelpers($db);

		$string = $helper->_adodb_quote_fieldname($db, $name);
