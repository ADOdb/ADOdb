...
class VariablesTest extends TestCase
{
    ...
    public function testQuotingExecute(): void
    {
        ...
        */
        ...
        $this->assertIsObject($success, 'Data insertion should not succed using Unquoted field and table names');

        $count = $this->db->getOne("SELECT COUNT(*) FROM `table_name`");

        $this->assertEquals(0, $count, 'Data insertion should not have succeeded using Unquoted field and table names');
        /*
        ...
        */
        ...
        $this->assertIsObject($success, 'Data insertion failed Using Quoted field and table names. The failing SQL was: ' . $sql);
    }
    ...
    public function testFetchMode(): void
    {
        ...
        */
        ...
        $this->assertArrayHasKey('id',$testRow, 'Row should have an id column');        

        $ADODB_FETCH_MODE = ADODB_FETCH_NUM;

        $testRow = $this->db->getRow($sql);

        $this->assertArrayHasKey(0,$testRow, 'Row should have a numeric column');
    ...
    public function testGetOneEof(): void
    {
        ...
        $this->assertEquals($test, false, 'getOne by default should return false when no row is found');
        ...
        $this->assertEquals($test, -1, 'getOne should now flag by -1 when no row is found');
    ...
    {   
        ...
        $this->assertEquals(11,$result->recordCount(), 'With ADODB_COUNTRECS set to true, the record count should be 11');        

        $ADODB_COUNTRECS = false;

        $result = $this->db->Execute($sql);
        
        $this->assertEquals(-1,$result->recordCount(), 'With ADODB_COUNTRECS set to false, the record count should be -1');
